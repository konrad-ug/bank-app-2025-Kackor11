Feature: Account registry
  ...
  Scenario: User is able to update name of already created account
    Given Account registry is empty
    And I create an account using name: "john", last name: "doe", pesel: "95092909876"
    When I update "name" of account with pesel: "95092909876" to "jonathan"
    Then Account with pesel "95092909876" has "name" equal to "jonathan"

  Scenario: Created account has all fields correctly set
    Given Account registry is empty
    When I create an account using name: "elvis", last name: "presley", pesel: "55010100000"
    Then Account with pesel "55010100000" exists in registry
    And Account with pesel "55010100000" has "name" equal to "elvis"
    And Account with pesel "55010100000" has "surname" equal to "presley"
    And Account with pesel "55010100000" has "balance" equal to "0.0"
